...
public class SignInformationActivity extends Activity implements OnClickListener{
	...
	//	顶层的信息按钮;
	private TextView btnBack,btnFunction,vD1,tvTopic,mState02;
	...
	private String 				 taskid,bid,gid,state,simg,folderPath,filePath,	tmpPath,sSize,wid,operkind;
			...
			// 手动上传;
				...
				if(!tmp.equals("")){
					...
					if(gid!=null){
						...
						mBuilder.create();
						mBuilder.show();
					}else Toast.makeText(mContext, "内容不正确", Toast.LENGTH_SHORT).show();
					//	图片;
				}else Toast.makeText(mContext, "请先扫描一维/二维码", Toast.LENGTH_SHORT).show();
			}
